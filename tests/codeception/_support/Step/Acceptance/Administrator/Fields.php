...
class Fields extends Admin
{
	...
	/**
	 * Method to fill the multiple field for custom field image, integer, list, sql, userfield and usergroup
	 *
	...
	/**
	 * Method to fill the directory field for custom field media
	 ...
	 * Attention: In media field directory is a text field -> use iFillFieldDirectoryWith
	 *
	 * @param   string  $directory  The directory
	 *
	...
	public function iFillFieldsForCreatingField($field_params)
	{
		...
		foreach ($field_params->getRows() as $index => $row)
		{
			if ($index !== 0)
			{
				...
				// Image, Integer, List, User, Usergrouplist
				if (($row[1] == "Imagelist (imagelist)"
				...
				// Media
				if (($row[1] == "Media (media)") and isset($row[25]) and ! empty($row[25]))
				{
